...
def render_animation(keypoints, keypoints_metadata, poses,  skeleton, fps, bitrate, azim, output, viewport,
    ...
    if keypoints_metadata['layout_name']=='coco':
        ...
        keypoints2D_label = ['HIP', 'R_HIP', 'R_KNEE', 'R_FOOT', 'L_HIP', 'L_KNEE', 'L_FOOT', 'SPINE','THORAX', 'NOSE', 'HEAD',
    ...
    for index, (title, data) in enumerate(poses.items()):
        ...
        #ax = fig.add_subplot(1, 1 + len(poses), index+2, projection='3d')
            ...
            if title == 'Reconstruction':
                ax.set_xlim3d([-1.5, 1])   #([-1, 1])
                ax.set_zlim3d([-1, 1])
                ax.set_ylim3d([-1.5, 1])
            else:
                ax.set_xlim3d([-0.05,0.05])        #([-0.03,0.03])     #([-radius/2, radius/2])
                ax.set_zlim3d([0.0,0.2])           #([-0.1,0.1])    #([0, radius])
                ax.set_ylim3d([-0.5,0.01])          #([-0.1,0.1])    #([-radius/2, radius/2])
            '''
            try:
                ax.set_aspect('equal')
    ...
    def update_video(i):
        ...
        if not initialized:
            ...
            for j, j_parent in enumerate(parents):
                ...
                col2 = 'yellow' if j in skeleton.joints_right() else 'green'   # jk edit  :  different colors for gt skeleton
                color = [col,col2]

                for n, ax in enumerate(ax_3d):
                    pos = poses[n][i]
                    #lines_3d[n].append(ax.plot([pos[j, 0], pos[j_parent, 0]],
                    lines_3d[n].append(ax_3d[0].plot([pos[j, 0], pos[j_parent, 0]],    # jk edit plot reconstruction and ground truth at the same plot
            ...
            for j, j_parent in enumerate(parents):
                ...
                for n, ax in enumerate(ax_3d):
                    ...
                    for joint in range(xs.shape[0]):
                        annot_3d[n][joint].set(text = '') #dict_keypoints[joint] + '(' + str(np.round(xs[joint], 2)) + ',' + str( np.round(ys[joint], 2)) + ',' + str(np.round(zs[joint], 2)) + ')')
    ...
    anim = FuncAnimation(fig, update_video, frames=np.arange(0, limit), interval=1000/fps, repeat=False,) #blit=True)
