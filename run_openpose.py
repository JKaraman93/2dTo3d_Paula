...
# -d custom -k myvideos -arc 3,3,3,3,3 -c checkpoint --evaluate pretrained_h36m_detectron_coco.bin --render --viz-subject input_video.mp4 --viz-action custom --viz-camera 0 --viz-video inference/input_dir_video/input_video.mp4 --viz-output output.mp4 --viz-size 6
# -e 2 -k cpn_ft_h36m_dbb -arc 3,3 -b 32 -str S1,S5  -a Walking -ste S11
# --render --viz-subject S11 --viz-action Walking --viz-camera 0 --viz-output output.gif --viz-size 3 --viz-downsample 2 --viz-limit 60
# -k cpn_ft_h36m_dbb -arc 3,3 -c checkpoint --evaluate epoch_2.bin --render --viz-subject S11 --viz-action Walking --viz-camera 0 --viz-output output.mp4 --viz-size 3 --viz-downsample 2 --viz-limit 60
# -d custom -k cpn_ft_h36m_dbb -arc 3,3,3,3,3 -c checkpoint --evaluate pretrained_h36m_detectron_coco.bin --render --viz-subject S11 --viz-action Walking --viz-camera 0 --viz-output output.mp4 --viz-size 6
# -d custom -k myvideos -arc 3,3,3,3,3 -c checkpoint --evaluate pretrained_h36m_detectron_coco.bin --render --viz-subject testMultipleViewsFront.mp4 --viz-action custom --viz-camera 0 --viz-video C:\Users\dimis\PycharmProjects\VideoPose\inference\input_dir_video\testMultipleViewsFront.mp4 --viz-output output.mp4 --viz-size 6
# -k cpn_ft_h36m_dbb -arc 3,3,3,3,3 -c checkpoint --evaluate pretrained_h36m_cpn.bin --render --viz-subject S11 --viz-action Walking --viz-camera 0 --viz-video "/path/to/videos/S11/Videos/Walking.54138969.mp4" --viz-output output.gif --viz-size 3 --viz-downsample 2 --viz-limit 60
# -k cpn_ft_h36m_dbb -arc 3,3,3 -c checkpoint --evaluate epoch_10.bin --render --viz-subject S11 --viz-action Walking --viz-camera 0 --viz-output output2.mp4 --viz-size 5 --viz-downsample 2 --viz-limit 60
# -d custom -k myvideos -arc 3,3,3 -c checkpoint --evaluate epoch_10.bin --render --viz-subject testMultipleViewsFront.avi --viz-action custom --viz-camera 0 --viz-video C:\Users\dimis\PycharmProjects\VideoPose\inference\input_dir_video\testMultipleViewsFront.avi --viz-output output.mp4 --viz-size 6
# -d custom -k myvideos -arc 3,3,3 -c checkpoint --evaluate epoch_6.bin --render --viz-subject testMultipleViewsFront.avi --viz-action custom --viz-camera 0 --viz-video C:\Users\dimis\PycharmProjects\VideoPose\inference\input_dir_video\testMultipleViewsFront.avi --viz-output output.mp4 --viz-size 6
# -k detectron_pt_coco -arc 3,3,3 -c checkpoint --evaluate epoch_6.bin --render --viz-subject S11 --viz-action Walking --viz-camera 0 --viz-output output2.mp4 --viz-size 6 --viz-downsample 4 --viz-limit 60
# 29.3.23 # -d custom -k myvideos -arc 3,3,3 -c checkpoint --evaluate epoch_6.bin --render --viz-subject testMultipleViewsFront.avi --viz-action custom --viz-camera 0 --viz-video C:\Users\dimis\PycharmProjects\VideoPose\inference\input_dir_video\testMultipleViewsFront.avi --viz-export out_cam_13cords --viz-output output.mp4 --viz-size 6
# 30.3.23 # -k cpn_ft_h36m_dbb -arc 3,3,3 -c checkpoint --evaluate epoch_10.bin --render --viz-subject S11 --viz-action Walking --viz-camera 0 --viz-output output2.mp4 --viz-size 5 --viz-downsample 4 --viz-limit 60
# 6.4.23 # -d custom -k detectron -arc 3,3,3,3,3 -c checkpoint --evaluate pretrained_h36m_detectron_coco.bin --render --viz-subject testExportV2Front.mp4 --viz-action custom --viz-camera 0 --viz-camera 0 --viz-video inference\input_dir_video\Paula_vid_\testExportV2Front.mp4 --viz-output inference\testExportV2Front\output.mp4 --viz-export inference\testExportV2Front\testExportV2Front_pred3d_cam.npy --viz-size 6

# 10.5.23 # -d custom -k openpose -arc 3,3,3,3,3 -c checkpoint --evaluate pretrained_h36m_cpn.bin  --render --viz-subject testExportV2Front.mp4 --viz-action custom --viz-camera 0 --viz-camera 0 --viz-video inference\input_dir_video\Paula_vid_\testExportV2Front.mp4 --viz-output output.mp4 --viz-export testExportV2Front_pred3d_cam.npy --viz-size 6
# -d custom -k detectron -arc 3,3,3,3,3 -c checkpoint --evaluate pretrained_h36m_detectron_coco.bin --render --viz-subject testExportV2Front.mp4 --viz-action custom --viz-camera 0 --viz-camera 0 --viz-video inference\input_dir_video\Paula_vid_\testExportV2Front.mp4 --viz-output output.mp4 --viz-export testExportV2Front_pred3d_cam.npy --viz-size 6
...
if args.keypoints in ['mpcoco', 'detectron']:
...
try:
    ...
    os.makedirs('inference/'+ args.viz_subject[:-4] + '/'+ args.keypoints+'_predictions')
except:
...
if args.dataset == 'h36m':
    ...
    dataset = CustomDataset('inference/'+ args.viz_subject[:-4] +'/'+ args.keypoints+ '.npz')
    # dataset = CustomDataset('data/data_2d_h36m_' + args.keypoints + '.npz')   # jk edit
else:
...
keypoints = np.load('inference/'+ args.viz_subject[:-4] +'/'+ args.keypoints+ '.npz',  allow_pickle=True)
...
if args.resume or args.evaluate:
    ...
    # if not checkpoint['model_traj']: #jk
    #    del checkpoint['model_traj']

    if args.evaluate and 'model_traj' in checkpoint:
        # if args.evaluate and 'model_traj' not in checkpoint:  # karaman edit/ test for vizualization if use model of your own
...
if args.render:
    ...
    # to adjust the axes according to paulas camera frame #jk
    prediction[:, :, 1] = -prediction[:, :, 1]  # jk
    prediction[:, :, 2] = - prediction[:, :, 2]  # jk
    # prediction = evaluate(gen, return_predictions=False) # to get metrics in inference mode
    ...
    if args.viz_export is not None:
        ...
        np.save('inference/'+vid[:-4]+'/'+ args.keypoints+'_predictions/' + args.viz_export, prediction)

    if args.viz_output is not None:
        ## jk world cordinates #3
        positions_3d = []
        if args.keypoints in ['mpcoco','detectron']:
            ground_truth_world = np.float32(np.load('inference/'+vid[:-4]+'/'+vid[:-4]+'_world_correctedhips.npy')) / 100  # cm to meters
        else:
            ground_truth_world = np.float32(np.load('inference/'+vid[:-4]+'/'+vid[:-4]+'_world.npy')) / 100  # cm to meters

        ground_truth = world_to_camera(ground_truth_world, R=cam['orientation'], t=cam['translation'])
        positions_3d.append(ground_truth)
        ##
        if ground_truth is not None:
            # Reapply trajectory
            trajectory = ground_truth[:, :1]
            # ground_truth[:, 1:] += trajectory   #comment the line for paula JK
        ...
        # ground_truth[:, :, 2] -= np.min(ground_truth[:, :, 2]) #jk

        # export coords in world space JK
        if args.viz_export is not None:
            file_name = 'inference/'+vid[:-4]+'/'+ args.keypoints+'_predictions/' +vid[:-4]+'_pred3d_world.npy'
            print('Exporting joint positions to', file_name)
            # Predictions are in camera space
            np.save(file_name, prediction)

        anim_output = {'Reconstruction': prediction}

        if ground_truth is not None and not args.viz_no_ground_truth:  # jk
        ...
        render_animation(input_keypoints, keypoints_metadata, anim_output,
                         dataset.skeleton(), dataset.fps(), args.viz_bitrate, cam['azimuth'], 'inference/'+vid[:-4]+'/'+ args.keypoints+'_predictions/' + args.viz_output,
                         limit=args.viz_limit, downsample=args.viz_downsample, size=args.viz_size,
                         input_video_path=args.viz_video, viewport=(cam['res_w'], cam['res_h']),
                         input_video_skip=args.viz_skip, )  # radius=4)

        # copy result video to static folder for handling from Dash
        shutil.copyfile('inference/'+vid[:-4]+'/'+ args.keypoints+'_predictions/' + args.viz_output, 'static/output_'+vid[:-4]+'_'+args.keypoints+'.mp4')
